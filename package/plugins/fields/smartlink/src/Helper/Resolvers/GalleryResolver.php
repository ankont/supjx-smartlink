...
final class GalleryResolver extends AbstractResolver
{
    ...
    public function resolve(array $payload): array
    {
        ...
        foreach ($items as $index => $item) {
            ...
            $classes = 'smartlink-gallery__item';
            $attributes = [
            ...
            if ($linkBehavior === 'lightbox-hook') {
                ...
                $attributes[] = 'data-smartlink-lightbox="1"';
            }

            if ($type === 'video') {
                $poster = $this->asMediaUrl((string) ($item['poster'] ?? ''));
                $thumb = $poster !== '' ? '<img src="' . $this->escape($poster) . '" alt="' . $this->escape($label) . '" loading="lazy">' : '<span class="smartlink-gallery__video-label">' . $this->escape($label !== '' ? $label : 'Video') . '</span>';
